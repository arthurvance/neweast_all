...
const IDEMPOTENCY_NON_CACHEABLE_STATUS_CODES_BY_ROUTE = new Map([
  ...
  [PLATFORM_USER_UPDATE_ROUTE_KEY, IDEMPOTENCY_NON_CACHEABLE_STATUS_CODES],
...
const createRouteTable = ({
  ...
  return {
    ...
    [PLATFORM_INTEGRATION_CONTRACT_CONSISTENCY_CHECK_ROUTE_KEY]: async () =>
      executeIdempotentAuthRoute({
        routeKey: PLATFORM_INTEGRATION_CONTRACT_CONSISTENCY_CHECK_ROUTE_KEY,
        execute: () =>
          runAuthRouteWithTrace(
            () =>
              handlers.platformCheckIntegrationContractConsistency(
                requestId,
                headers.authorization,
                getRouteParams(),
                body || {},
    ...
    [PLATFORM_USER_UPDATE_ROUTE_KEY]: async () =>
      executeIdempotentAuthRoute({
        routeKey: PLATFORM_USER_UPDATE_ROUTE_KEY,
        execute: () =>
          runAuthRouteWithTrace(
            () =>
              handlers.platformUpdateUser(
                requestId,
                headers.authorization,
                getRouteParams(),
                body || {},
