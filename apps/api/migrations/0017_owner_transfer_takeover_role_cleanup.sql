...
SET @owner_takeover_role_collision_count = (
  ...
  JOIN platform_roles prc ON BINARY prc.role_id = BINARY migration_tenants.resolved_role_id
  WHERE NOT (
    prc.scope = 'tenant'
...
SELECT
  ...
  NULL
FROM tmp_owner_takeover_legacy_tenants migration_tenants
LEFT JOIN platform_roles prc ON BINARY prc.role_id = BINARY migration_tenants.resolved_role_id
WHERE prc.role_id IS NULL;

UPDATE platform_roles prc
JOIN tmp_owner_takeover_legacy_tenants migration_tenants
...
INSERT IGNORE INTO tenant_membership_roles (
  membership_id,
  role_id,
  created_by_user_id,
  updated_by_user_id
...
JOIN (
  ...
  FROM tmp_owner_takeover_legacy_membership_targets migration_memberships
  LEFT JOIN tenant_membership_roles mr
...
SET ut.can_view_user_management = membership_snapshot.can_view_user_management,
    ...
    ut.can_view_role_management = membership_snapshot.can_view_role_management,
    ut.can_operate_role_management = membership_snapshot.can_operate_role_management,
