...
export default function TenantUserManagementPage({
    ...
    async (members = []) => {
      ...
      const roleEntries = await Promise.all(
        members.map(async (member) => {
          ...
          try {
            const payload = await api.getUserRoles(membershipId);
    ...
    async (membershipId, latestActionOverride = null) => {
      ...
      try {
        const [detailPayload, rolePayload] = await Promise.all([
          ...
          api.getUserRoles(normalizedMembershipId)
        ]);
        const latestAction =
  ...
  const openMemberProfileModal = useCallback(async (record) => {
    ...
    try {
      const rolePayload = await api.getUserRoles(normalizedMembershipId);
  ...
  const handleCreateMember = useCallback(async () => {
    ...
    try {
      ...
      createdMemberPayload = await api.createUser({
        phone: values.phone
      });
  ...
  const memberTableRequest = useCallback(
    async (params) => {
      ...
      if (sourceMembers.length === pageSize) {
        const lookAheadPayload = await api.listUsers({
          page: page + 1,
          pageSize: 1
        });
        const hasNextPage =
