...
- [x] 任务 3：落实“禁用即受限、启用即恢复”的组织域访问控制（AC: 1, 2）
  ...
  - [x] 明确并实现受限策略：禁用组织后，相关组织域访问不可继续通过（`tenant options`/`tenant scoped authorize` 至少一处强约束，建议双重约束）。
  - [x] 启用组织后，符合条件成员可恢复组织域访问。
  - [x] 关键状态变更触发会话收敛（`session_version` 递增与会话/refresh 处理策略）并与现有认证链路约束一致。
...
- 组织禁用后必须落实“组织域访问受限”这一业务结果，不能只更新 `orgs.status` 而不影响鉴权路径。
- 关键状态变更需要落实会话一致性策略（`session_version` 收敛），避免“组织已禁用但历史会话仍可用”的安全漏洞。
...
- AC1：启用 -> 禁用
  ...
  - 组织域访问被限制（至少覆盖一个 tenant 入口 + 一个 tenant 受保护路由）
- AC2：禁用 -> 启用
...
- [x] 组织状态关键变更触发会话收敛（session_version + 会话/refresh 收敛）
