...
- [x] 组织选择与切换接口（AC: 2,3,4）
  - [x] 新增组织候选查询与选择接口（建议：`/auth/tenant/options`、`/auth/tenant/switch`）
...
- 已新增 `/auth/tenant/options`、`/auth/tenant/switch`、`/auth/tenant/switch`，并在服务端会话中持久化与切换 `active_tenant_id`。
