...
**Success Check**
- 组织角色授权仅命中 `tenant.*`。
- 同一组织内多角色按并集生效，角色变更可即时生效，成员权限表现与角色一致。
...
- FR26
  - 需求陈述: 系统可以维护平台域与组织域独立的权限命名空间与授权边界（平台域仅使用 `platform.*`，组织域仅使用 `tenant.*`，`platform.*` 不得提升组织域能力）。
  - 验收口径: 采用统一模板 AC1/AC2/AC3，且三项全部通过。
  - 测量方法: 发布前执行自动化回归、权限负向回归与审计链路抽样。
  - 追踪来源: User Journeys + MVP Scope + Success Criteria。
...
- FR31
  - 需求陈述: 系统可以阻止仅平台身份且无组织关系的用户进入组织工作台，并阻止仅组织身份且无平台权限的用户进入平台工作台。
  - 验收口径: 采用统一模板 AC1/AC2/AC3，且三项全部通过。
  - 测量方法: 发布前执行自动化回归、权限负向回归与审计链路抽样。
  - 追踪来源: User Journeys + MVP Scope + Success Criteria。
...
- FR49
  - 需求陈述: 系统可以按角色并集计算用户有效权限（平台域按平台角色并集，组织域按同一组织内组织角色并集，不使用显式 deny）。
  - 验收口径: 采用统一模板 AC1/AC2/AC3，且三项全部通过。
  - 测量方法: 发布前执行自动化回归、权限负向回归与审计链路抽样。
  - 追踪来源: User Journeys + MVP Scope + Success Criteria。
...
- FR70
  - 需求陈述: 系统可以在组织域鉴权中使用服务端确认的有效租户上下文进行最终访问判定，并仅解析 `tenant.*` 权限（忽略 `platform.*`）。
  - 验收口径: 采用统一模板 AC1/AC2/AC3，且三项全部通过。
  - 测量方法: 发布前执行自动化回归、权限负向回归与审计链路抽样。
  - 追踪来源: User Journeys + MVP Scope + Success Criteria。
